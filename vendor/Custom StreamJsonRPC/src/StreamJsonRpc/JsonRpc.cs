...
#pragma warning restore CA2000 // Dispose objects before losing scope
                ...
                else
                {
                    ...
                    else
                    {
                        if (this.TraceSource.Switch.ShouldTrace(TraceEventType.Warning))
                        {
                            foreach (string requestArgumentName in request.ArgumentNames)
                            {
                                Console.WriteLine(requestArgumentName);
                            }
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        public EventHandler<dynamic> UnknownReply { get; set; }
...
#pragma warning restore CA1031 // Do not catch general exception types
                ...
                else if (rpc is IJsonRpcMessageWithId resultOrError)
                {
                    ...
                    {
                        this.UnknownReply?.Invoke(this, result?.Result);
                    }

                    
                    //Ignore when we don't know the request ID to allow streamdeck to be used along side this SDK
                    //else
                    //{
